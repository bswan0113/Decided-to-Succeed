...
namespace Core
{
    public class SceneTransitionManager : IInitializable
    {
        ...
        public async UniTask FadeAndLoadScene(string sceneName) // void -> async UniTask로 변경
        {
            if (_isTransitioning)
            {
                CLogger.LogWarning($"Scene transition already in progress. Ignoring request to load '{sceneName}'.");
                return;
            }
        ...
        private async UniTask FadeAndLoadSceneAsync(string sceneName, CancellationToken cancellationToken = default)
        {
            ...
            CLogger.LogInfo($"[SceneTransitionManager] Scene transition started for '{sceneName}'.");
            ...
            try
            {
                ...
                CLogger.LogDebug("[SceneTransitionManager] Fade Out complete.");
                ...
                CLogger.LogInfo($"[SceneTransitionManager] Scene '{sceneName}' loaded and activated successfully. New active scene: {CurrentSceneName}");

                // --- Fade In ---
                await FadeAsync(0f, cancellationToken);
                CLogger.LogDebug("[SceneTransitionManager] Fade In complete.");
            }
            catch (OperationCanceledException)
            {
                CLogger.LogWarning($"[SceneTransitionManager] Scene transition for '{sceneName}' was cancelled.");
                transitionFailed = true;
            }
            catch (Exception ex)
            {
                CLogger.LogError($"[SceneTransitionManager] Failed to load scene '{sceneName}'. Exception: {ex.Message}");
                transitionFailed = true;
            }
            finally
            {
                ...
                CLogger.LogInfo($"[SceneTransitionManager] Scene transition for '{sceneName}' finished (status: {(transitionFailed ? "Failed" : "Completed")}).");
            }
        }
        ...
        private async UniTask FadeAsync(float targetAlpha, CancellationToken cancellationToken = default)
        {
            if (_view.FadeImage == null)
            {
                CLogger.LogError("FadeImage is null. Cannot perform fade.");
                return;
            }
